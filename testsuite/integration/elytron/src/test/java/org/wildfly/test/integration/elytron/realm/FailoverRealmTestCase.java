...
/**
 * Tests the {@link FailoverRealm} within the Elytron subsystem.
 *
 * @author Ondrej Kotek
 */
...
//        testCredentialBasedRealms(webAppUrl);
//    }
...
//    }
    ...
    public void testTokenRealmUnvailable(@ArquillianResource URL webAppUrl) throws Exception {
        try (CloseableHttpClient httpClient = HttpClientBuilder.create().setRedirectStrategy(REDIRECT_STRATEGY).build()) {
            cleanAuditLog();
            String result = doHttpRequestTokenAuthn(httpClient, webAppUrl, createJwtToken("userA", "issuer.wildfly.org"), SC_OK);
            assertEquals(INDEX_PAGE_CONTENT, result);
            assertTrue("SecurityRealmUnavailableEvent should be logged", isSecurityRealmUnavailableEventLogged());
    ...
    private static boolean isSecurityRealmUnavailableEventLogged() throws Exception {
        List<String> lines = Files.readAllLines(auditLog.toPath(), StandardCharsets.UTF_8);
    ...
    private static void cleanAuditLog() throws Exception {
        try (PrintWriter writer = new PrintWriter(auditLog)) {
            writer.print("");
        }
    }
    ...
    static class ServerSetup extends AbstractElytronSetupTask {
        ...
        protected ConfigurableElement[] getConfigurableElements() {
            ...
            FileAuditLog fileAuditLog = FileAuditLog.builder()
                    .withName("audit_log_for_failover_realm")
                    .build();
            auditLog = new File(fileAuditLog.getPath());
            configurableElements.add(fileAuditLog);
            ...
            new File(unavailableFileSystemRealmPath).setExecutable(false, false);
            ...
            configurableElements.add(JdbcSecurityRealm.builder("unavailable_jdbc_realm")
                    .withPrincipalQuery("ExampleDS", "invalid SQL").build()
                    .build());
            ...
            configurableElements.add(PropertiesRealm.builder()
                    ...
                    .withUser(UserWithAttributeValues.builder()
                            .withName("user1")
                            .withPassword("password1") // this is expected to be hashed
                            .withValues("user")
                            .build())
                    .build());
            ...
            configurableElements.add(FailoverRealm.builder("failover_realm_both_unavailable")
                    .withDelegateRealm("unavailable_filesystem_realm")
                    .withFailoverRealm("unavailable_filesystem_realm")
                    .build());
            configurableElements.add(SimpleSecurityDomain.builder()
